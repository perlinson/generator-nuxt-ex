...
module.exports = class extends Generator {
  ...
  prompting() {
    ...
    var apiDir = this.config.get("apiDir") || "server/api";
    var storeDir = this.config.get("storeDir") || "store";
    ...
    var prompts = [
      ...
      },
      {
        ...
        name: "adminMethods",
        message:
          "Which methods are only accessible by authenticated admin users?",
        default: [],
          ...
          return choices.map(function(choice) {
            if (props.masterMethods.indexOf(choice.value) !== -1) {
              return _.assign({}, choice, {
                disabled: "Accessible only with master key"
              });
            }
      ...
      },
      {
        ...
        choices: function(props) {
          ...
          return choices.map(function(choice) {
            ...
            if (props.adminMethods.indexOf(choice.value) !== -1) {
              return _.assign({}, choice, {
                disabled: "Accessible only by admin users"
              });
            }
      ...
      },
      {
        ...
        name: "modelFields",
        message:
          "Which fields the model will have? (comma separated, do not include id)",
        when: function(props) {
      ...
      },
      {
        ...
        message: function(props) {
          return (
            "Do you want to store in a field the user who created the " +
            props.kebab +
            "?"
          );
        },
        when: function(props) {
          return (
            props.generateModel &&
            props.userMethods &&
            props.userMethods.indexOf("POST") !== -1
          );
        }
      },
      {
      ...
      },
      {
        ...
        name: "getList",
        message:
          "Do you want the retrieve methods (GET) to have the form { rows, count } ?",
        default: false,
        when: function(props) {
    ...
    return this.prompt(prompts).then(
      function(props) {
        ...
        if (this.props.lowerSuffix) {
          this.props.camel =
            _.lowerCase(this.props.camel) + this.props.lowerSuffix;
        }
        ...
        if (
          props.userField &&
          this.props.modelFields.indexOf(props.userField) !== -1
        ) {
          this.props.modelFields.splice(
            this.props.modelFields.indexOf(props.userField),
            1
          );
        }
      }.bind(this)
  ...
  writing() {
    ...
    var sourcepath = function(filename) {
      return path.join(props.apiDir, filename);
    };

    var filepath = function(filename) {
      return path.join(props.apiDir, props.kebab, filename);
    };

    // Var storeFile = path.join(props.storeDir, props.kebab, ".js");

    copyTpl(
      tPath("store/index.js"),
      dPath("store/" + props.kebab + ".js"),
      props
    );

    copyTpl(
      tPath("pages/index.vue"),
      dPath("pages/" + props.kebab + ".vue"),
      props
    );

    copyTpl(
      tPath(sourcepath("controller.js")),
      dPath(filepath("controller.js")),
      props
    );
    ...
    copyTpl(tPath(sourcepath("index.js")), dPath(filepath("index.js")), props);
    copyTpl(
      tPath(sourcepath("index.test.js")),
      dPath(filepath("index.test.js")),
      props
    );

    if (props.generateModel) {
      copyTpl(
        tPath(sourcepath("model.js")),
        dPath(filepath("model.js")),
        props
      );
      copyTpl(
        tPath(sourcepath("model.test.js")),
        dPath(filepath("model.test.js")),
        props
      );
    }
    ...
    //     return (
    //       callee.object.name === "router" && callee.property.name === "use"
    //     );
    //   });
